"""
LoRA selection handler for KGen.

This module provides functions and classes for handling LoRA selection
and configuration in the KGen application.
"""
...
from kgen.lora.lora_loader import (
...
class LoRASelectionHandler:
    ...
    def get_lora_config(self, model_type: str = "flux-schnell", lora_mode: str = "all") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "kgen", "config/lora_config.json")
            
            with open(config_path, 'r') as f:
                lora_configs = json.load(f)
            
            if model_type not in lora_configs:
                from kgen.console import print_error
            ...
            from kgen.console import print_error
            print_error(f"Error loading LoRA configuration: {str(e)}", "Please check kgen/lora_config.json")
            sys.exit(1)
