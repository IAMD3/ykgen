"""
Group mode image generation for KGen.
...
from . import ComfyUIWaiClient
from kgen.lora.lora_selector import select_loras_for_scenes, select_loras_for_all_scenes_optimized
...
def generate_images_for_scenes_group_mode_optimized(
    ...
    for i, scene in enumerate(scenes):
        ...
        try:
            ...
            elif model_type == "wai-illustrious":
                # Use Wai model
                client = ComfyUIWaiClient(lora_config=story_lora_config)
                scene_image_paths = client.generate_scene_images(single_scene, output_dir)
            else:
                # Use Flux model
...
def generate_images_for_scenes_all_mode(
    ...
    try:
        if model_type == "illustrious-vpred":
            ...
            # Import here to avoid circular imports
            from .comfyui_image_wai import generate_wai_images_for_scenes
            image_paths = generate_wai_images_for_scenes(
                scenes=scenes,
                lora_config=lora_config,
                output_dir=output_dir
            )
